...
test('Master Bot User 5: Post & Link Collector', async ({ page, request }) => {
  const configPath = path.join(process.cwd(), 'user5.json');
  ...
  if (await emailInput.isVisible({ timeout: 7000 })) {
    console.log("🔑 [User 5] กำลังเริ่มกรอกข้อมูล Login...");
  ...
  for (const postItem of allPosts) {
    for (const groupID of (postItem.groupID || [])) {
      ...
      try {
        console.log(`🚀 [User 5] เข้ากลุ่ม: ${gID}`);
        ...
        if (await postTrigger.isVisible({ timeout: 10000 })) {
          ...
          console.log(`✍️ [User 5] กำลังพิมพ์ Caption...`);
          ...
          if (await closePreviewBtn.isVisible()) {
            console.log("🎯 [User 5] พบ Link Preview! กำลังกดปิด...");
            await closePreviewBtn.click();
            await page.waitForTimeout(2000);
          } else {
            console.log("ℹ️ [User 5] ไม่พบปุ่มปิด Preview หรือ Preview ไม่แสดง");
          }
