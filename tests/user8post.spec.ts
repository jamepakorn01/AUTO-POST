...
test('Master Bot User 8: Post & Link Collector', async ({ page, request }) => {
  const configPath = path.join(process.cwd(), 'user8.json');
  ...
  if (await emailInput.isVisible({ timeout: 7000 })) {
    console.log("🔑 [User 8] กำลังเริ่มกรอกข้อมูล Login...");
  ...
  for (const postItem of allPosts) {
    for (const groupID of (postItem.groupID || [])) {
      ...
      try {
        console.log(`🚀 [User 8] เข้ากลุ่ม: ${gID}`);
        ...
        const postTrigger = page.locator(
          'div[role="button"]:has-text("เขียนอะไรสักหน่อย"), ' +
          'div[role="button"]:has-text("Write something"), ' +
          'div[role="button"]:has-text("สร้างโพสต์สาธารณะ")'
        ).first();
        ...
        if (await postTrigger.isVisible({ timeout: 10000 })) {
          ...
          // ✅ วนลองคลิกกล่องพิมพ์จนกว่า Pop-up Error จะหาย
          let editorReady = false;
          for (let attempt = 1; attempt <= 10; attempt++) {

            const errorPopup = page.locator(
              'div:has-text("เกิดข้อผิดพลาดขึ้น"), ' +
              'div:has-text("ข้อผิดพลาดทางเทคนิค")'
            ).first();

            if (await errorPopup.isVisible({ timeout: 1000 }).catch(() => false)) {
              console.log(`⚠️ [User 8] พบ Pop-up Error (ครั้งที่ ${attempt}) รอ 15 วินาที...`);
            ...
            if (await captionEditor.isVisible({ timeout: 3000 }).catch(() => false)) {
              ...
              if (isFocused) {
                console.log(`✅ [User 8] กล่องพิมพ์พร้อมแล้ว (ครั้งที่ ${attempt})`);
                editorReady = true;
                break;
              }
            }

            console.log(`🔄 [User 8] รอกล่องพิมพ์... (ครั้งที่ ${attempt})`);
            await page.waitForTimeout(3000);
          }

          if (!editorReady) {
            console.log(`❌ [User 8] กล่องพิมพ์ไม่พร้อม ข้ามกลุ่ม ${gID}`);
          ...
          console.log(`✍️ [User 8] กำลังพิมพ์ Caption...`);
          ...
          const closePreviewBtn = page.locator(
            'div[aria-label="ลบพรีวิวลิงก์ออกจากโพสต์ของคุณ"], ' +
            'div[aria-label="Remove link preview from your post"]'
          ).first();
          
          if (await closePreviewBtn.isVisible()) {
            console.log("🎯 [User 8] พบ Link Preview! กำลังกดปิด...");
            await closePreviewBtn.click();
            await page.waitForTimeout(2000);
          } else {
            console.log("ℹ️ [User 8] ไม่พบปุ่มปิด Preview หรือ Preview ไม่แสดง");
          }
